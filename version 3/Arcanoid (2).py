...
class ball:
    ...
    def box(s,bx):
        ...
        if s.inr(s.x,bx.x-bxw/2,bx.x+bxw/2) and s.inr(s.y-s.r,bx.y-bxh/2,bx.y+bxh/2):
            ...
            bx.y=-9999
class box:
    def __init__(s,x,y,a,t):
        s.x=x
        s.y=y
        s.a=a
        s.c="#%s%s%s"%tuple(('00'+hex(randint(15,255))[2:])[-2:]for i in range(3))
        s.t=t
        s.o=w.create_image(x,y,image=bxm[t][a])
class platform:
...
bxm=[[PhotoImage(file='box%d.gif'%(j+1),format = 'gif -index %d'%i)for i in range([35,37,49][j])]for j in range(0,3)]
for i in range(35):
    bxw=bxm[0][i].width()
...
while True:
    ...
    if time()-at>=0.05:
        for i in range(4):
            for j in range(10):
                a=boxes[i][j].a+1
                if boxes[i][j].t==0:a=a%35
                if boxes[i][j].t==1:a=a%37
                if boxes[i][j].t==2:a=a%49
                if a==1:a=1
                boxes[i][j].a=a
                w.itemconfig(boxes[i][j].o,image=bxm[boxes[i][j].t][a])
        at=time()
