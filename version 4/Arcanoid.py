...
class ball:
    ...
    def box(s,bx,i,j): #шарик внутри коробки
        ...
        if s.inr(s.x-s.r,bx.x-bxw/2,bx.x+bxw/2) and s.inr(s.y,bx.y-bxh/2,bx.y+bxh/2):
            ...
            if bx.t!=3:#если 4 тип коробки, то сменяется другой
                w.delete(bx.o)
                bx.x=-9999
                bx.y=-9999
            else:
        ...
        if s.inr(s.x,bx.x-bxw/2,bx.x+bxw/2) and s.inr(s.y+s.r,bx.y-bxh/2,bx.y+bxh/2):
            ...
            if bx.t!=3:#если 4 тип коробки, то меняется тип коробки, она не убирается
                w.delete(bx.o)
                bx.x=-9999
                bx.y=-9999
            else:
...
while True:
    ...
    (lambda rr:(w.move(bal.o,*rr),w.move(bal.t,*rr)))(bal.move())#движение текста и шарика
    ...
    if time()-at>=0.05: #если прошло больше 50мс, запускаем следующий кадр
        for i in range(4): #меняем анимацию
            for j in range(10):
                if boxes[i][j].t!=3:
                    a=boxes[i][j].a+1
                    if boxes[i][j].t==0:
